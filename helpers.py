...
def containment(ngram_array):
    ...
    count = 0
    intersect_idx = []
    for idx in range(len(ngram_array[0])):
        if ngram_array[0][idx] == ngram_array[1][idx]:
            count +=1
            intersect_idx.append(idx)
#     print(f'no. intersect indexes: {len(intersect_idx)}')
#     print(f'intersect count: {count}')
#     print(f'sum answer: {np.sum(ngram_array[0])}')
    return count / np.sum(ngram_array[0])
